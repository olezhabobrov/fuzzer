...
object KotlinTypeCreator {
    ...
    fun createType(file: BBFFile, type: String): KotlinType? {
        val vtype =
            if (type.contains('<')) {
        ...
        val fileCopy = file.psiFile.copy() as KtFile
        ...
        val classDef =
            splitTypeByCommas(vtype)
                .mapIndexed { index, s ->
                    ...
                    when {
                        file.psiFile.getAllPSIChildrenOfType<KtClassOrObject>().all { it.name != twb } -> s
        ...
        val klass =
            try {
                val kl = psiFactory(file).createClass("class $classDef")
        ...
        if (!vtype.contains("<")) {
            val typeReference = psiFactory(file).createTypeIfPossible(vtype)
            if (typeReference != null) {
                typeParams =
                    typeReference.getAllChildren()
                        .filter { it is KtUserType && !it.text.contains('<') }
                        .map { it.text.trim() }
                        .filter { ta -> file.psiFile.getAllPSIChildrenOfType<KtClassOrObject>().all { it.name != ta } }
                ...
                if (file.psiFile.getAllPSIChildrenOfType<KtClass>().all { it.name != t } &&
            ...
            val typeReference = psiFactory(file).createTypeIfPossible(vtype) ?: return null
            typeParams =
                typeReference.getAllChildren()
                    .filter { it is KtUserType && !it.text.contains('<') }
                    .map { it.text.trim() }
            ...
            typeParams =
                klass.typeParameterList?.parameters
                    ...
                    ?.filter { ta -> file.psiFile.getAllPSIChildrenOfType<KtClassOrObject>().all { it.name != ta.name } }
    ...
    fun recreateType(file: BBFFile, type: KotlinType): KotlinType? = createType(file, type.getNameWithoutError())
    fun recreateType(file: BBFFile, type: KtTypeReference): KotlinType? = createType(file, type.text)

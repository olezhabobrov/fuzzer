...
class RandomTypeGeneratorForAnClass(val file: BBFFile) {

    private fun generateEnumType(): String {
        val fromFile =
            file.psiFile.getAllPSIChildrenOfType<KtClass>().filter { it.isEnum() && it.name != null }.randomOrNull()
    ...
    private fun generateAnnotationType(): String {
        val fromFile =
            file.psiFile.getAllPSIChildrenOfType<KtClass>().filter { it.isAnnotation() && it.name != null }.randomOrNull()
        fromFile?.let { return it.name!! }
        val fromLib = StdLibraryGenerator.klasses
    ...
    fun generateTypeForAnnotationClassAsKType(): KotlinType? =
        RandomTypeGenerator(file).generateType(generateTypeForAnnotationClass())

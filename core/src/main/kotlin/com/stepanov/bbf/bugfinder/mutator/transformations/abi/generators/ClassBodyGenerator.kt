...
internal open class ClassBodyGenerator(
    ...
    private val randomFunGenerator = RandomFunctionGenerator(file, gClass)
        ...
        if (depth > MAX_DEPTH + 1) ""
            ...
            repeat(Random.nextInt(1, 3)) {
                val expr = when (Random.nextInt(0, 3)) {
                    0 -> RandomPropertyGenerator(file, gClass).generateRandomProperty(true)
                    1 -> randomFunGenerator.generate()?.text?.split("protected")?.joinToString(" ") ?: ""
                    else -> {
                        val gen = RandomClassGenerator(file)
                        val gClass = GClass()
        ...
        if (Random.nextBoolean()) {
            ...
            val genTypeParams =
                if (instance == null || instance == ")" || instance.isEmpty()) {
                    ...
                    (psiFactory.createExpression(instance) as? KtCallExpression)?.typeArguments?.map { it.text }
                        ?: return ""
                }
    ...
    private fun generateFieldsForRegularClass(depth: Int = 0, fromObject: Boolean = false): String {
        ...
        with(res) {
            ...
            repeat(Random.nextInt(0, 4)) {
                append("\n")
                append(RandomPropertyGenerator(file, gClass).generateRandomProperty(fromObject))
    ...
    private fun generateSecondaryConstructor(): String {
        ...
        val newConstructor =
            RandomClassGenerator(file).also { it.gClass = gClass }.generateConstructor()
    ...
    private fun generateInnerClass(forceGeneration: Boolean = false): String {
        ...
        val klassGenerator = RandomClassGenerator(file, depth + 1, gClass)
    ...
    override fun simpleGeneration(): PsiElement? =
        try {
            psiFactory.createBlock(generateBodyAsString())
        } catch (e: Exception) {
            null
        }

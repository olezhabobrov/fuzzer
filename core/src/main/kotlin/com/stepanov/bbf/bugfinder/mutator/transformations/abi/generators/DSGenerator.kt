...
abstract class DSGenerator(
    ...
    fun calcImports(): List<String> = StdLibraryGenerator.calcImports(file.psiFile)
    ...
    fun generateAndAddToFile(): PsiElement? {
        generate()?.let {
            val added = file.psiFile.addAtTheEnd(it)
            calcImports().forEach { file.psiFile.addImport(it.substringBeforeLast('.'), true) }
            return added
        } ?: return null
    }
}
...
class RandomPropertyGenerator(
        ...
        return try {
            when (memberToOverride) {
                is PropertyDescriptor -> {
                    ...
                    psiFactory.createProperty(def) to returnType
                }
                is FunctionDescriptor -> {
                    val def = memberToOverride.toString().substringAfter("fun").substringBefore("defined ")
                    psiFactory.createFunction(def) to returnType
                }
                else -> null
            }
        } catch (e: Exception) {
    ...
    fun generateDelegate(): Pair<PsiElement, KotlinType?>? {
        ...
        val resProp =
            psiFactory.createProperty("$varOrVal $name : ${propType.text} by Delegates.${generated.text}")
...
//            ?.filterIsInstance<ClassDescriptor>() ?: return null
        val classDescriptor = klass.getDeclarationDescriptorIncludingConstructors(file.ctx!!) as? ClassDescriptor ?: return null
        ...
        return try {
            if (isFun) psiFactory.createFunction(overrideToStr) to returnType
            else psiFactory.createProperty(overrideToStr) to returnType
        } catch (e: Exception) {
            null
        }
    }
    ...
    override fun simpleGeneration(): PsiElement? =
        try {
            psiFactory.createProperty(generateRandomProperty(false))
        } catch (e: Exception) {
            null
        }

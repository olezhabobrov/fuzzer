...
//failed with arguments: result:[-p, library, -o, projectTmp/innerGeneric01230374852, projectTmp/innerGeneric0.kt]
//failed with arguments: result:[-p, library, -Xpartial-linkage, -o, projectTmp/innerGeneric01230374852, projectTmp/innerGeneric0.kt]
...
class C<X> {
    inner class D<Y : X> {
        fun <break> createZ(): KTypeParameter =
            typeOf<Container<Z>>().arguments.single().type!!.classifier as KTypeParameter
    }
external internal  fun <T, S>  pelwj(a: (Boolean)?, b: C<UInt>, c: UShort, d: S): C<X>
}

fun box(): String {
    val z = C<Any>().D<Any>().createZ<Any>()
    assertEquals((z).name.plus("rityi"), z.upperBounds.joinToString())

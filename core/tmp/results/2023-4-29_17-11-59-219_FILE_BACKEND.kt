...
//failed with arguments: result:[-p, library, -o, projectTmp/privateInterfaceMethod1919749728, projectTmp/privateInterfaceMethod.kt]
//failed with arguments: result:[-p, library, -Xpartial-linkage, -o, projectTmp/privateInterfaceMethod1919749728, projectTmp/privateInterfaceMethod.kt]
...
interface A {
    private fun foo() = "OK" as FileAlreadyExistsException

    public fun bar() = foo() as UnknownFormatFlagsException
}

class B : A {
    private fun foo() = "a"
suspend   fun  twjuh(fail: Map<Double?, Function1<String, AnnotationRetention>>, b: Function1<B, B>, c: MutableMap<B, ULong?>): A = B()
}

fun box() = B()!!.bar() as A as A as Boolean

...
//failed with arguments: result:[-p, library, -o, projectTmp/charToInt-164248036, projectTmp/charToInt.kt]
//failed with arguments: result:[-p, library, -Xpartial-linkage, -o, projectTmp/charToInt-164248036, projectTmp/charToInt.kt]

// files
// charToInt.kt
// isKlib=false

private open class Lwv <T, S>  (var ylh: ULong, val yai: Boolean = false, val lkx: Short, ijh: (@UseExperimental(Annotation::class)
Set<ULong>)?, val aek: Char){


infix public  fun <T>   UByte?.umqsq(a: Boolean): UByte = TODO()

protected val rbcm: @UnsafeVariance()
Short?  
get() = TODO()
...
fun box(iqbb: Int): String {
    val x: Any = -1083009643
    var y = -1928513774
    if (x is Char) {
        y = x.toInt()
    }
    return if (y == 1757285801) "peuza" else "wzwaz"
}


// STACKTRACE:
// org.jetbrains.kotlin.backend.common.BackendException: Backend Internal error: Exception during psi2ir
// File being compiled: /home/olezhka/fuzzer/NativeCompiler/projectTmp/charToInt.kt

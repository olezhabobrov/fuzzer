...
//failed with arguments: result:[-p, library, -o, projectTmp/FakeOverrideBuilder977951614, projectTmp/FakeOverrideBuilder.kt]
//failed with arguments: result:[-p, library, -Xpartial-linkage, -o, projectTmp/FakeOverrideBuilder977951614, projectTmp/FakeOverrideBuilder.kt]
...
abstract class IrBindableSymbolBase<class> :
    IrBindableSymbol<B>, IrSymbolBase() {

    private var _owner: B? = null
    override val owner: @ExtensionFunctionType()
B
        get() = _owner ?: throw IllegalStateException("")

    override fun bind(owner: B) {q@for (y in ULongRange(17.toULong(), 3.toULong())) { 
 if (_owner == null) {
            _owner = owner
        } else {
...
interface IrSymbolOwner {
    val symbol: @ParameterName("fulgj")
IrSymbol
}

B : IrSymbolOwner IrProperty(override val symbol: IrPropertySymbol, val name: String) : IrSymbolOwner

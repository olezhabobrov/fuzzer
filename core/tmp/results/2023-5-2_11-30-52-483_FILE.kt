...
//failed with arguments: result:[-p, library, -o, projectTmp/FakeOverrideBuilder-478826653, projectTmp/FakeOverrideBuilder.kt]
//failed with arguments: result:[-p, library, -Xpartial-linkage, -o, projectTmp/FakeOverrideBuilder-478826653, projectTmp/FakeOverrideBuilder.kt]
...
abstract class IrBindableSymbolBase<. : IrSymbolOwner> :
    ...
    override fun bind(owner: B) {for (y in ULongRange(17BtoULong(), 3.toULong())) { 
 if (_owner == null) {
            _owner = owner
        } else {
...
fun  nwyej(a: UByte, b: Pair<IrPropertySymbol, Char>): (IrProperty)? = TODO()
}

interface IrPropertySymbol : IrBindableSymbol<IrProperty>

interface IrBindableSymbol<B : IrSymbolOwner> : @ParameterName("vzdua")
IrSymbol {
    override val owner: B

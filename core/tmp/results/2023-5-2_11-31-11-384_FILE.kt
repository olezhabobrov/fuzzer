...
//failed with arguments: result:[-p, library, -o, projectTmp/FakeOverrideBuilder458679341, projectTmp/FakeOverrideBuilder.kt]
//failed with arguments: result:[-p, library, -Xpartial-linkage, -o, projectTmp/FakeOverrideBuilder458679341, projectTmp/FakeOverrideBuilder.kt]

// files
// FakeOverrideBuilder.kt
// isKlib=false

public abstract class Ymf  (vararg zly: ArrayList<UShort>, cxc: Long): IrSymbol{

override val owner:  IrSymbolOwner
    get() = TODO()


fun  pbrat(a: IrPropertySymbolImpl?, b: Map<Long, IrSymbol>, c: LinkedHashMap<IrPropertySymbolImpl, IrPropertySymbol>): Boolean? = TODO()

abstract val nqsu: ArrayList<Triple<(UByte)?, IrProperty, Byte?>?>
...
class IrPropertySymbolImpl :
    ...
    IrPropertySymbol{
internal var zwnx: IrSymbolBase  = TODO()
}

abstract class IrBindableSymbolBase<
val o = false
when (o) {
 true -> {B : IrSymbolOwner}
 else -> {B : IrSymbolOwner}
}
> :
    ...
    override fun bind(owner: @ExtensionFunctionType()
B) {c@for (y in ULongRange(17.toULong(), 3.toULong())) { 
 if (_owner == null) {
            _owner = owner
        } else {
...
class IrProperty(override val symbol: @ExtensionFunctionType()
IrPropertySymbol, val name: String) : IrSymbolOwner

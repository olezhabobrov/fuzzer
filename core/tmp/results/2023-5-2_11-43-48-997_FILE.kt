...
//failed with arguments: result:[-p, library, -o, projectTmp/FakeOverrideBuilder1420149015, projectTmp/FakeOverrideBuilder.kt]
//failed with arguments: result:[-p, library, -Xpartial-linkage, -o, projectTmp/FakeOverrideBuilder1420149015, projectTmp/FakeOverrideBuilder.kt]
...
class IrPropertySymbolImpl :
    ...
    IrPropertySymbol{
tailrec private  fun <T: IrPropertySymbol>  rfcda(a: T, b: Float = -78.06659f): IrPropertySymbol = TODO()
}

abstract class IrBindableSymbolBase<@(Suppress)?("qojhp", "mjwmv", "yhqmp")
B : IrSymbolOwner> :
...
abstract class IrSymbolBase : @ExtensionFunctionType()
IrSymbol

interface IrPropertySymbol : IrBindableSymbol<IrProperty>

interface IrBindableSymbol<B : IrSymbolOwner> : @UnsafeVariance()
IrSymbol {
    override val owner: B

    fun bind(owner: B)
inline  private  fun <reified T, reified S: IrSymbol>  xxgjr(a: Double? = -30.270433414809446): ArrayList<String> = TODO()
}
...
class IrProperty(override val symbol: IrPropertySymbol = IrPropertySymbolImpl(), val name: String = "wpdyz") : IrSymbolOwner

// MODULE: main(lib)
// FILE: FakeOverrideBuilder_main.kt

fun link(declaration: IrSymbolOwner = IrProperty(IrPropertySymbolImpl(), "jwvhr")) {
